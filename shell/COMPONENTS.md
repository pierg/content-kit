...
Chrome (`lib.js`, built from `/content/catalog.json` and always inserted **outside** `<main>` — the text a reader annotates is exactly the text the page carries). Three zones: the library on the left and the page panel on the right, each of which the reader can close (the top bar's two buttons, or `[` and `]`; kept in the browser, on a wide screen; both are drawers on a narrow one), and between them the **canvas**, which is the page's own.
- **Library** (left) — the site's name, a search button (⌘K / Ctrl-K / `/` anywhere opens the palette), Home · Browse · Chronicle and the repo's kit.json `links`, then the library as a tree: **by topic** when pages declare `<meta name="topic">` (each topic → its hub as *Overview*, then its pages by genre), **by genre** otherwise (one group per genre that owns a directory, in the genre table's order), then the record's files. Groups fold; what a reader opens stays open. At its foot, the theme (auto · light · dark), kept in the browser.
- **Top bar** — the library's button; breadcrumbs (site / topic / genre), or a book's chapters as pills on a chapter page; "Updated … · N min read" while the page panel is closed; the page panel's button.
- **Page panel** (right; open by default from 1240px) — a **Page** tab: *On this page* (the `<h2>`/`<h3>` outline, following the scroll); *Page*: its kind and topic, its status, when it was created and updated (from the catalog), its reading time, its tags as links into Browse; *Linked from* (from `/content/backlinks.json`); *Links to* (the page's own internal links, each once, named from the catalog). No markup is needed for any of it. When the engine serves annotations, an **Annotations** tab holds the threads on the page (below). A layer may add a tab: `hbPanel.pane(name, label)` returns its pane; `hbPanel.show(name)`, `hbPanel.close()` and `hbPanel.state()` drive the panel, and `document` hears `hb:panel` (`{open, tab}`) when either changes.
...
The shell's own pages (browse, chronicle, record) carry `<body class="hb" data-hb-app>`: wide, sans, no page panel. A layer's shell page is treated the same way because it is served under `/shell/`.

**A page that designs its own layout** carries `<body class="hb" data-hb-canvas>`: it gets all of the canvas, the sans face and no measure, and keeps the page panel. Its words stay in its markup — a script may arrange, reveal and animate them, never write them — because annotations are anchored, and search indexes headings and definitions, from the page file. The canvas is a size container named `hb-canvas`: adapt to the room the page has with `@container hb-canvas (max-width: …)`, not `@media`, since the panels share the window with it.
...
This is the default layout, the one a page gets unless it is a canvas page. Prose — the direct children of `<main>` that are `h1`–`h6`, `p`, `ul`, `ol`, `dl`, `blockquote`, `details`, `.note`, `.q`, `.law`, `.anchors` (and the same inside an unclassed `div`, `section` or `article`) — sits in a measure of `--measure` (~70 characters a line). Everything else — `figure`, `table`, `pre`, `.cols`, `section.card`, a widget — breaks out to `--wide`. So write prose as plain children of `<main>` and it reads well; give a widget its own container and it gets the room.

Two vendored variable faces (SIL OFL, `vendor/fonts/`): **Source Serif 4** for reading (`--font-body`, `--font-serif`) and **Inter** for headings and chrome (`--font-sans`); code is `--font-mono`. A page may set its body in sans with `<body class="hb" data-hb-font="sans">`. The reader's own size is the browser's zoom, which scales everything; size a widget's text in `rem` or `em`. SVG text keeps the system face, so a hand-laid figure never reflows.

A page that is not current opens its lede with `<b>Status: X</b>` (HISTORICAL · PARKED · RETIRED · FROZEN · DRAFT), rendered as a pill (a class on the author's own `<b>`; the text is untouched). A current page states nothing: LIVE is the default, and the page panel shows it.
...
Every page shows who links to it in the page panel (*Linked from*), from `/content/backlinks.json` — a lint-generated reverse index of every internal `href` in the library — and where it links (*Links to*), read from the page itself. No markup is needed. A page written before 0.5.0 may carry the list in its body (`<ul data-backlinks>`): it is still filled, in place of the panel's, and the lint names it as no longer needed.

## Search and browse

Two surfaces, one index. The **palette** (⌘K / Ctrl-K / `/`, or the library's search button) jumps to a page, a topic, a tag or a shell page as you type. **Browse** (`/shell/search.html`, engine chrome) lists the library narrowed by words, kind, topic and tag — the filters live in the URL (`?q=` `&kind=` `&topic=` `&tag=`), so a tag chip or a breadcrumb links straight to a view — newest first when there are no words. Both read `/content/search-index.json` (title, `.sub` line, headings, `defn` blockquote, `<meta name="tags">`), Browse merged with every extra index kit.json `"indices"` declares (a personal library's federated catalog of sibling repos, say). Every word must match somewhere; titles weigh most.
