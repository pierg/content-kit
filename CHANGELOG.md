...
- **The book gate runs ordinary widget code.** `verify_book.mjs` runs a chapter's inline scripts against a wider stand-in DOM: `createElementNS`, text nodes, fragments, observers, timers, events, `window` as the script's global, and an inert answer for any element method it does not model. A widget that works in a browser is no longer failed for building SVG the standard way. A syntax error, an undefined name and an exception the page's own logic throws still fail, and the selftest plants one of each. Nothing to move across.
