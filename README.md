...
| `/shell/record.html?p=<file.md>` · `/shell/chronicle.html` · `/shell/dashboard.html` | the record, rendered — as raw markdown, as a **timeline** (story / timeline / experiments), and as an opt-in **dashboard** (status board); `content/chronicle.json` and `content/ladder.json` are generated, see below |
...
The **dashboard** (`/shell/dashboard.html`) is the same record as a *status board* — the current phase, the open questions and their kill criteria, the experiment pipeline, findings by status, the claims they rest on, and the recent decisions. It is generated into `content/ladder.json` alongside the chronicle (an extractor may add a `ladder(root, cfg)` for its own F-/C-/Q- vocabulary; the rest is generic), regenerated by `ckit lint` and drift-checked by `ckit check`. It is **opt-in**: set `"home": "dashboard"` in `lab.json` to generate it and serve it as the front door at `/` (or `"dashboard": true` to generate it without changing the door). A repo that does neither is entirely unaffected — no manifest, no catalog change, same gate. `"home"` may instead be a repo-relative path to an authored page (a file, or a directory with an `index.html`) to make *that* the front door — `/` 302s to its canonical URL — while `"dashboard": true` keeps the dashboard generated a click away; `ckit check` fails loud on a `home` that resolves to nothing.
